...
- ルート遷移は Nuxt `pageTransition` の `memo-page-fade`（300ms / `out-in`）でフェードアウト→フェードインを行い、一覧↔詳細に一貫したアニメーションを提供する。
...
- `/` のメモ一覧は SPA ビルドでも `ClientOnly` プレースホルダを表示し、クライアント初期化後にローカル IndexedDB データへ差し替えることで初期読込中の骨組み UI を維持する。
...
- メモ/カテゴリー詳細は編集後500msで自動保存され、保存状態（保存中・保存済み・エラー）をヘッダーに表示。初回編集時にドラフトを生成し、BroadcastChannel/Storageイベント経由でタブ間同期する。
- 詳細画面フッターは「完了」ボタンのみを配置し、保存エラー時はステータスエリアの「再試行」から手動で自動保存を再実行する。
- 検索バーは `AppPanelSearch` の虫眼鏡アイコンクリックで即座に入力へフォーカス（一覧以外では戻る動作に切り替え）、アクセシビリティを担保する。
- メモ一覧のカテゴリ説明は本文が空の場合は非表示とし、ダミーテキストを出さずに視覚ノイズを抑制する。
...
- BroadcastChannel 非対応ブラウザでは `localStorage` の `storage` イベントを利用したフォールバックチャネルで同期する。
...
- 手動E2E（Playwright MCP）: 2025-11-01 ルート遷移フェード（300ms）と戻る操作の整合性、警告ログ非発生を確認。
